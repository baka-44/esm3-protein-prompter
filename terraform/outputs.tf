...
output "rf3_job_name" {
  value = google_cloud_run_v2_job.rf3_worker.name
}
...
#   GCP_PROJECT, GCP_REGION, PROTEINREDESIGN_JOB_NAME, PROTEINREDESIGN_RF3_JOB_NAME,
#   PROTEINREDESIGN_INPUTS_BUCKET, PROTEINREDESIGN_OUTPUTS_BUCKET, PROTEINREDESIGN_WEIGHTS_BUCKET
# PROTEINREDESIGN_RF3_JOB_NAME routes RF3 presets (#8) to the rf3-worker job (D11).
output "frontend_env_hint" {
  value = {
    GCP_PROJECT           = var.project_id
    GCP_REGION            = var.region
    PROTEINREDESIGN_JOB_NAME       = var.job_name
    PROTEINREDESIGN_RF3_JOB_NAME   = var.rf3_job_name
